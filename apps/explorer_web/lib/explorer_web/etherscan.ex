defmodule ExplorerWeb.Etherscan do
  ...
  @wei_type %{
    type: "wei",
    definition:
      "The smallest subdenomination of #{@coin}, and thus the one in which all integer values of the currency are counted, is the Wei. One #{
        @coin
      } is defined as being 10<sup>18</sup> Wei.",
  ...
  @transaction %{
    ...
    fields: %{
      ...
      isError: %{
        type: "error",
        enum: ~s(["0", "1"]),
        enum_interpretation: %{"1" => "error", "0" => "ok"}
      },
  ...
  @account_balance_action %{
    name: "balance",
    description: "Get Ether balance for address",
    required_params: [
      %{
        key: "address",
  ...
  @account_balancemulti_action %{
    name: "balancemulti",
    description: "Get Ether balance for multiple addresses",
    required_params: [
      %{
        key: "address",
