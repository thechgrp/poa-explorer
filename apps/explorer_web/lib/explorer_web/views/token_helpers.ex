defmodule ExplorerWeb.TokenHelpers do
  ...
  """
  def token_transfer_amount(%TokenTransfer{token: token, amount: amount}) do
    do_token_transfer_amount(token, amount)
  end

  defp do_token_transfer_amount(%Token{type: "ERC-20"}, nil) do
    "--"
  end

  defp do_token_transfer_amount(%Token{type: "ERC-20", decimals: nil}, amount) do
    CurrencyHelpers.format_according_to_decimals(amount, 0)
  end

  defp do_token_transfer_amount(%Token{type: "ERC-20", decimals: decimals}, amount) do
    CurrencyHelpers.format_according_to_decimals(amount, decimals)
  end

  defp do_token_transfer_amount(%Token{type: "ERC-721", id: id}, _amount) do
    "TokenID [#{id}]"
  end

  defp do_token_transfer_amount(_token, _amount) do
    nil
  end
